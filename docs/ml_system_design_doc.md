...
###### Что есть данные для платформы?
В полном варианте системы ожидается псевдо мультимодальность - возможность загружать в агента данные следующих типов:
1. Текст (запрос пользователя)
2. Документ свободного формата (например, .pdf, .txt, .xlsx и т.п.)
3. Изображение (в том числе, с комбинацией текстового запроса)
4. Аудио (в том числе, с комбинацией текстового запроса)

В MVP предполагается работа только с текстовым запросом. Т.к. в скоуп итерции входит исключительно RAG,
то процесс будет выглядеть следующим образом:
1. Векторизация запроса.
2. Guardrail запроса (новая фича, вне бизнес требований)
3. Векторный поиск топ K.
4. В случае нахождения релевантных (выше трешхолда вероятности) чанков - ретривер + генерация.
5. В случае не нахождения релевантных чанков - статический ответ.

Предполагается одно внутреннее состояние прототипа без дополнительной генерации промежуточных и изменённых выходных.
Состав состояния приблизительно такой:
1. Список сообщений (Человек, Агент, Тулза) с редьюсером типа ADD.
2. Trace ID (для развязки от LangSmith)

На выходе должен получится текстовый ответ в одном из двух видов:
1. Релевантная вопросу генерация со ссылкой на документ (оценивается заказчиком визуально).
2. Сообщение-заглушка на случай, если релевантных чанков найдено не было.

В обоих случаях тип выходных данных - текст.

###### Препроцесс
Только в контексте текущей итерации. Предобработка входного запроса не производится.

###### Guardrails
Производится тестирования guardrails:
1. Langchain PII - блокируем передачу персональной информации в контур.
2. Human-in-the-loop как guardrail - подтверждение пользователем использования тулзов.
3. Before agent guardrails - детерминированные правила - словари запрещённых слов.
4. After agent guardrails - недетерменированные правила - легковесная или квотированная модель с промптом безопасности.

Пример промпта безопасности (возможен ENUM SO):

f"""Evaluate if this response is safe and appropriate for corporate communications.
        Respond with only 'SAFE' or 'UNSAFE'.

##### Этап 2 Прямая связка с RAG
...
- Тестирование Guardrails производится в этом этапе.

##### Этап 3 Условная связка с RAG и MCP
...
##### Этап 5 Создание CLI/Terminal интерфейса для добавления агентов
